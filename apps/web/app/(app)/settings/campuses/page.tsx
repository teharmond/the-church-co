...
import NewCampusDialog from "./new-campus-dialog";
...
const campuses = [
  {
    title: "Hills Campus",
    description: "/hills-campus",
    path: "/settings/campuses/1",
  },
  {
    title: "City Campus",
    description: "/city-campus",
    path: "/settings/campuses/2",
  },
];
    ...
    <div className="mt-6 w-full space-y-6 p-5">
      <SettingsSection>
        <div className="flex justify-between">
          <SettingsHeader>
            <SettingsTitle>Campuses</SettingsTitle>
            <SettingsDescription>Manage your campuses.</SettingsDescription>
          </SettingsHeader>
          <NewCampusDialog />
        </div>
        <div className="flex items-center gap-2">
          <Input placeholder="Search campuses" className="h-10 w-full" />
          <Button className="h-10 rounded-md" variant="outline">
            View Archived
          </Button>
        </div>
        <SettingsContent>
          {campuses.map((setting, index) => {
            return (
              <SettingsRow
                key={setting.path}
                larger
                ...
                className={cn(
                  ...
                  index === campuses.length - 1 && "rounded-b-md",
                )}
              >
                <SettingsRowDetails>
